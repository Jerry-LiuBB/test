...
from config import CanonConfig, OutputConfig, RobotConfig
from dataio.pose_io import read_pose_rows
from robot.driver import MockRobotDriver, RealManRobotDriver
from scanner.executor import execute_scan
from scanner.planner import generate_circle_waypoints


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="D435i + Canon + Robot arm scan")
    sub = parser.add_subparsers(dest="mode", required=True)

    gen = sub.add_parser("generate", help="Generate circular waypoints and scan")
    gen.add_argument("--out-dir", default=None, help="Override output directory from config")
    ...
    replay.add_argument("--out-dir", default=None, help="Override output directory from config")
...
def main() -> None:
    ...
    if robot_config.enabled:
        robot = RealManRobotDriver(
            ip=robot_config.ip,
            port=robot_config.port,
            timeout=robot_config.timeout,
            dof=robot_config.dof,
        )
        print(f"Using RealMan robot at {robot_config.ip}:{robot_config.port}")
    else:
        robot = MockRobotDriver()
        print("Using Mock robot (no real hardware)")
    ...
    camera = DualCamera(
        ...
        realsense_fps=30,
        canon_enabled=canon_config.enabled,
        canon_ip=canon_config.camera_ip if canon_config.enabled else None,
        canon_port=canon_config.port,
        canon_save_dir=canon_config.save_dir,
    )
    robot.connect()

    try:
        out_root = Path(args.out_dir) if args.out_dir else output_config.root_dir
        ...
        if args.mode == "generate":
            ...
            records = execute_scan(
                robot, camera, waypoints, out_dir, speed=args.speed, trigger_path=trigger_path
            )
            print(f"Done: generated+captured {len(records)} points into {out_dir}")

        elif args.mode == "replay":
            rows = read_pose_rows(Path(args.poses_txt))
            waypoints = [{"xyz": r["xyz"], "rpy": r["rpy"]} for r in rows]
            records = execute_scan(
                robot, camera, waypoints, out_dir, speed=args.speed, trigger_path=trigger_path
            )
            print(f"Done: replayed+captured {len(records)} points into {out_dir}")
    finally:
        camera.close()
        robot.disconnect()
