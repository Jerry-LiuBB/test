...
def build_parser() -> argparse.ArgumentParser:
    ...
    gen.add_argument("--out-dir", default="output/session_generate", help="Output directory")
    ...
    replay.add_argument("--out-dir", default="output/session_replay", help="Output directory")
...
def main() -> None:
    ...
    try:
        if args.mode == "generate":
            ...
            records = execute_scan(robot, camera, waypoints, Path(args.out_dir), speed=args.speed)
            print(f"Done: generated+captured {len(records)} points into {args.out_dir}")

        elif args.mode == "replay":
            rows = read_pose_rows(Path(args.poses_txt))
            waypoints = [{"xyz": row["xyz"], "rpy": row["rpy"]} for row in rows]
            records = execute_scan(robot, camera, waypoints, Path(args.out_dir), speed=args.speed)
            print(f"Done: replay-captured {len(records)} points into {args.out_dir}")
    finally:
        camera.close()
        robot.disconnect()
